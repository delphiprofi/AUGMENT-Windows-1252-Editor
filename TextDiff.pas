...
*                   htt p://www.cs.arizona.edu/people/gene/                     *
*                   htt p://www.cs.arizona.edu/people/gene/PAPERS/diff.ps       *
*                                                                              *
*******************************************************************************)
...
{$ENDIF}
  ...
  public
    ...
    function Execute(Const LeftStr,RightStr : AnsiString) : TDiff;overload;
    function IsSame(AProzent : byte) : boolean;
...
function TDiff.Execute(const LeftStr, RightStr: AnsiString): TDiff;
begin
  Execute(pchar(LeftSTR), pchar(RightStr), length(LeftStr), length(RightStr));                      { TODO 1 -oXEFrank -cAnsiString in PWideChar : XE }
  Result := Self;
end;
...
function TDiff.IsSame(AProzent: byte): boolean;
var
  Summ,
  Different : Integer;
begin
  Different := DiffStats.adds+DiffStats.deletes+DiffStats.modifies;
  Summ      := Different + DiffStats.matches;
  Result := ((Different * 100 / Summ) < (100-AProzent))
end;
...
begin
  ...
  // application.processmessages; // Removed for console application
