...
{.$DEFINE DRVTest} //VORSICHT, IST NUR FÜR TESTS!!!!!
{$ENDIF}
...
Uses Windows,Sysutils,xmldom, XMLIntf, msxmldom, XMLDoc, StdCtrls,
     ...
     kopfstellekba, eAktenSystem.Intf
, uxJustizVersion
      ...
      if lSgOWriter.GetSafeID(lMySafeID)
        then begin
               lSgOWriter.NachrichtenKopf(TGVSHandler.DoCreateGUID32,'','',lMySafeID,aTargetSafeID,'')
                         .Sachgebiet(cSachgebietMRA,'XMeld Anfrage',cInstancebehoerde,aDRNr) // FL 21.10.22 Sachgebiet für MRA FL 06.09.22
 ...
 begin
  ...
  try
   ...
   if FileExists(TempPath+XML) then begin
    ...
    {$ELSE}
    //01.03.16:
// BW 14.11.22 nicht nur für User_Status=0    if (User_STatus=0) and (f755802l1.Combo1.Visible) and (f755802l1.Combo1.ItemIndex=10) then begin
    ...
    {später
         ...
         if F755802l1.MultiAGS.Checked then
          Res:=f755802l1.MRAInfo+' an EGVP / eBO übergeben'
         else
          Res:='An EGVP / eBO übergeben!';
        end;
       end
       else begin
